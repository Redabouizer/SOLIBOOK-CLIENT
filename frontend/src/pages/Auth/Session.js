...
export function setTokenWithExpiration(key, token) {
    ...
    const keyStr = "sangat-aman-12345"; // Encryption key
    const iv = CryptoJS.lib.WordArray.random(16); // Random initialization vector
    const encryptedToken = CryptoJS.AES.encrypt(token, keyStr, { iv: iv }).toString();
...
export function getTokenWithExpiration(key) {
    ...
    const keyStr = "sangat-aman-12345"; // Same encryption key
